...
namespace Lab3.Clases
{
    public class HuffmanCoder
    {
            ...
            (string rPath, string wPath)
        {
            ...
            Dictionary<char, byte[]> PrefixCodes = Huffman(CharactersProbabilites.ToList());/*Ask why afther this method the list changes*/
            ...
            (BinaryReader br, ref double charcount)
        {
            ...
            while (br.BaseStream.Position != br.BaseStream.Length)//while used to read all the file
            {
        ...
        private Dictionary<char, byte[]> Huffman
            (List<Node> CharProb)
        {
            while (CharProb.Count > 1)//while there is more than one node on the list
            {
                ...
                Aux.LeftChild = CharProb[0];
                Aux.RightChild = CharProb[1];
                CharProb.RemoveAt(0);
                CharProb.RemoveAt(0);
            ...
            (Node Root, ref Dictionary<char, byte[]> PCs, List<byte> PrefixCode)
        {
            ...
            {
                PrefixCode.Add(0);
                PrefixCodTransversal(Root.LeftChild, ref PCs, PrefixCode);
                PrefixCode.RemoveAt(PrefixCode.Count() - 1);
            }
            if (Root.RightChild != null)
            {
                PrefixCode.Add(1);
                PrefixCodTransversal(Root.RightChild, ref PCs, PrefixCode);
                PrefixCode.RemoveAt(PrefixCode.Count() - 1);
            }
        }//Untested
        ...
        private void GetProbabilities(List<Byte> table, ref Dictionary<char, Byte[]> dictionary)
        {
            ...
            dictionary = Huffman(Frecuency);
        }
        private void getdata(ref List<Byte> table, ref int numByte, ref int numCharacter)
        {
            //remueve los datos de cantidad de bytes y cantidad de caracteres que originalmente contenia el archivo
